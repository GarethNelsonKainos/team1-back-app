...
export const createJobRoleSchema = z.object({
  roleName: z.string().min(3, 'Role name must be at least 3 characters').max(100),
  ...
  jobSpecLink: z.string().url('Invalid SharePoint URL')
    .startsWith('https://kainossoftwareltd.sharepoint.com', 'Must be a Kainos SharePoint link'),
  openPositions: z.number().int().min(1),
  locationIds: z.array(z.number().int().positive()).min(1, 'At least one location required'),
